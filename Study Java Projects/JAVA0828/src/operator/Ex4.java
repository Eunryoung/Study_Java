...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 *   => 연산의 방향이 우 -> 좌 로 진행됨
		 *      ex) x = y = 3 의 경우에는 3을 y 에 대입하고, y 를 x 에 대입
...
//		b = b + a; // 10 + 10 = 20이 변수 b에 저장됨(b에 a값을 누적)
		...
		// b 와 a 의 뺄셈 결과를 다시 변수 b에 저장
//		b = b - a; // 20 - 10 = 10이 변수 b에 저장됨
		b -= a; // b = b - a;
		System.out.println(b);
		
		// b와 a의 곱셈 결과를 다시 변수 b에 저장
//		b = b * a; // 10 * 10 = 100이 다시 변수 b에 저장
		b *= a; // b = b * a;
		System.out.println(b);
		
		// b와 a의 나눗셈 결과(몫)를 다시 변수 b에 저장
//		b = b / a; // 100 / 10 = 10이 다시 변수 b에 저장
		b /= a; // b = b / a;
		System.out.println(b);
...
//		b1 = b1 + 10; // byte + int = int + int = int
		// byte 타입 b1에 int 값인 정수 20을 넣으려니 오류 발생
		//=> 결과값을 다시 byte 타입으로 변환해야함
		b1 = (byte)(b1 + 10);
		...
		b1 += 10; // b1 = b1 + 10;
		System.out.println(b1);
		// byte 타입으로 변환할 필요없이 확장 대입연산자를 사용하면 오류 발생 X
