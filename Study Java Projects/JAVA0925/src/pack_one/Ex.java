...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - java.lang 패키지는 유일하게 import 없이도 사용 가능한 패키지
		 * 	 => 그 외의 모든 패키지는 반드시 위치 정보가 필요함!
		 * - import 문은 패키지 키워드 보다 아래쪽, 클래스 선언부 보다 위쪽에
...
//		java.util.Random r;
		...
		// => pack_two.ExPack ex2 = new pack_two.ExPack(); // 클래스명마다 앞에 패키지명을 명시해주는게 가장 이상적!
		// => ExPack cannot be resolved to a type
		// => 오류 발생! 인스턴스가 생성되지 않음!
		// => Ex.java가 위치한 pack_one 패키지 내에 ExPack.java가 위치하지 않기 때문
		// => import 문을 통해 ExPack.java가 있는 pack_two, pack_three 패키지를 등록해줘야 한다!
		// => Ctrl + space 누를시 자동 완성으로 생성
