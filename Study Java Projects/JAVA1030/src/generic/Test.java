...
public class Test {
	...
	public static void main(String[] args) {
		...
		System.out.println(p1); // Person 클래스에서 오버라이딩 된 toString() 메서드가 생략
		System.out.println(p2); // Person 클래스에서 오버라이딩 된 toString() 메서드가 생략
...
//			System.out.println(objArr[0].name); // Person 타입 변수 접근 불가
...
//			Person p = objArr[i]; // 다운캐스팅 필요
		...
		/*
		 * 2. Collection API(e.g. ArrayList) 를 활용하여
		...
		// 문자열 "임은령" 추가
//		list.add("임은령"); // Object 타입이므로 Person이 아닌 타입도 추가 가능
		// => Person 객체 형태로 꺼내서 사용하는 시점에서 문제가 발생할 수 있다!
		
		// 일반 for문을 통한 객체 출력
//		for(int i = 0; i < list.size(); i++) {
//			Person p = list.get(i);
			// => 제네릭 타입을 사용하지 않아 list는 Object로 되어있음
			// => Object 타입을 Person 타입으로 대입하려고 하면 컴파일 에러 발생하며
			//	  다운캐스팅이 필요함을 알 수 있음
//			Person p = (Person)list.get(i);
//			System.out.println(p);
			// => 실행을 시키면 85번 라인에서 에러가 발생!
			// =>  java.lang.ClassCastException: class java.lang.String
			// 	   cannot be cast to class generic.Person
			// => "임은령" 의 String 형태는 Person 객체로 변경할 수 없음을 알 수 있다!
			// => 위의 코드 중 "list.add("임은령");을 주석처리하고 다시 실행하면 오류가 없어진다!
...
//		}
		 ...
		 * - 또한, 제네릭 타입 Person 타입으로 타입이 고정되어 있으므로
		 *   Object 타입으로 업캐스팅이 일어나지 않음
		 *   => 데이터를 꺼내는 시점에서도 Person 타입 그대로 사용 가능
		 *      (별도의 타입 체크 또는 다운캐스팅 작업이 불필요)
		 */
		
		// <Person> 제네릭을 사용하는 ArrayList(list2) 를 생성
