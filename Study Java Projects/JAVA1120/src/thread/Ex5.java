...
public class Ex5 {
	...
	public static void main(String[] args) {
		 ...
		 *   => 우선순위는 1 ~ 10 까지 범위의 정수를 사용
		 *   => 자주 사용되는 우선순위 값을 Thread 클래스의 상수로 제공
		 *      MAX_PRIORITY : 10(높음)
		 *      NORM_PRIORITY : 5(보통) -> 기본 값
		 *      MIN_PRIORITY : 1(낮음)
		 *      
		 ...
		 *      타이머가 만료되지 않아도 강제로 쓰레드를 깨울 수 있음
		 * - sleep() 메서드 호출 시 반드시 try ~ catch 블록 처리 필수!
		 * - 특정 쓰레드의 실행 시간에 대한 간격을 설정하거나,
		 *   우선 순위에 따라 실행되지 못하는 쓰레드의 실행권한을 부여하기 위해
		 ...
		 */
		 ...
		 System.out.println("t1 쓰레드 우선순위 : " + t1.getPriority()); // 1 출력
		 System.out.println("t2 쓰레드 우선순위 : " + t2.getPriority()); // 10 출력
