...
public class Ex6 {
	...
	public static void main(String[] args) {
		...
		WithdrawThread iBanking = new WithdrawThread("인터넷 뱅킹", account, 3000);
		WithdrawThread mBanking = new WithdrawThread("모바일 뱅킹", account, 3000);
		iBanking.start();
		mBanking.start();
...
class Account {
	int balance;
	// balance 라는 같은 데이터에 복수의 쓰레드에서 동시에 접근을 할때
	// synchronized 키워드로 동기화 메서드로 만들어서 일관성 유지! 
...
//	public int withdraw(int amount) {
		String threadName = Thread.currentThread().getName(); // 쓰레드명 저장 가능
