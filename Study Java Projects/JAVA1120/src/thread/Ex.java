...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *      e.g. 동영상을 재생하면서 웹 페이지 표시, 음악 재생하는 것
		 *      
		 * 쓰레드(Thread)
		 * - 프로세스 내에서 작업의 최소 단위
		 * - 하나의 프로세스 내에는 최소 한 개의 쓰레드가 동작함(= Single Thread)
		 *   => 동시에 수행 가능한 작업은 단 하나 뿐이다.
		 * - 하나의 프로세스 내에서 동시에 수행 가능한 작업을 늘리려면
		 *   멀티쓰레드(Multi Thread)로 구현해야 한다!
		 *   e.g. 메신저 내에서 파일 전송과 동시에 메시지 송/수신하는 것
		 *    
		 */
		...
		nt2.run(); // B작업이 시작되고, B작업이 50만번 실행이 끝나면
		nt3.run(); // C작업이 실행됨
...
class NoThread {
	...
	public NoThread(String name, int count) { // 파라미터 생성자 정의
		super();
		this.name = name;
		this.count = count;
	}
