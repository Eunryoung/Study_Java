...
//			System.out.println(array[i]);
			...
			if(max < array[i]) { 
				...
				System.out.println(array[i] + "가(이) " + max + "보다 더 크지 않다.");
			}
		}
		 ...
		 * 비교 대상 값으로 배열 내의 데이터를 갖고 시작하는것이 좋다!
		 * e.g. 음수끼리의 최대값을 계산하는데 초기값으로 0을 설정하는 경우
		...
		// 주의! 0번의 데이터를 이미 최대값으로 저장했기 때문에
		// 0번 인덱스를 제외한 나머지 값만 비교하면 됨(1번부터 반복)
