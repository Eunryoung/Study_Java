...
//		b = 100;
		...
		b = 127;
//		b = 128; // 오류 발생! byte 타입에 저장 가능한 수 -128 ~ +127 까지
		...
		s = 32767;
//		s = 32768;
		// => 오류발생! short 타입 저장 가능한 수 -32768 ~ +32767 까지
...
//		i = 2147483648L; 
		...
		// => i는 int 타입이지만 2147483648L;은 long 타입
		...
		i = 5; // 오류 발생 하지않음
//		i = 5L; // 오류 발생
		// => 5L은 long타입이 되어 int 타입인 i에 대입 불가
		// => 정수 5는 int 범위에 포함되지만, L 붙이면 저장 불가!
		
		// 실수 데이터타입
		double d = 3.14; // 실수형 데이터타입(double) 변수 d 에 3.14 초기화
//		float f = 3.14; // 오류 발생!
		// => 접미사가 없으면 double 타입으로 인식하므로 float 타입 변수 저장 불가!
		float f = 3.14f;
