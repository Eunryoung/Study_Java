...
import java.util.Calendar; // import 문으로 java.util 클래스 명시
...
public class Ex {
	...
	public static void main(String[] args) {
		...
		ci2.setCarModelName(CarInfoWithConstant.CAR_MODEL_NAME_SONATA); // 차량모델
		ci2.setColor(CarInfoWithConstant.COLOR_PERFECT_BLACK); // 색상
		
		// 값을 비교할 때도 상수명만으로 비교하면 오류 발생 확률 없음
...
//		Calendar cal = new Calendar(); // abstract 키워드 붙은 클래스는 인스턴스 생성 불가
		Calendar cal = Calendar.getInstance(); // getInstance() 이용해 참조변수 형태로 접근
		System.out.println("올해는 몇 년도? " + cal.get(Calendar.YEAR) + "년");
