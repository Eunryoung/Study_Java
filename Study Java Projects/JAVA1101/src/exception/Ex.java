...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 		예외가 발생하면 JVM에 의해 해당 예외 객체들을 전달 받아
...
//		System.out.println(arr[5]);
		...
		// out of bounds for length 3 at exception.Ex.main(Ex.java:50) => 50번 라인에서 예외 발생
		
//		String str = null;
//		System.out.println(str.length());
		// => 객체의 주소를 참조하지 않는 참조변수(null) 접근 시 NullPointerException 예외 발생
		// => java.lang.NullPointerException: Cannot invoke "String.length()" because "str" is null
		//    at exception.Ex.main(Ex.java:56) => 56번 라인에서 예외 발생
		
		System.out.println("프로그램 종료!"); // 위에서 예외 발생 시 실행되지 않는 코드
		...
		try {
			...
			int num = 1;
			System.out.println(3 / num); // ArithmeticException 예외 발생 위치
			// 만약 이 지점에서 예외 발생 시 나머지 try 블록의 코드들은
			// 실행되지 못하고 바로 catch 문으로 이동
			
			int[] arr = {1, 2, 3};
			System.out.println(arr[2]); // ArrayIndexOutOfBoundsException 예외 발생 위치
			// ArithmeticException 외에 다른 예외도 처리해야할 경우
			// 또 다른 catch 블록을 추가할 수 있다!
			// => 이 때, catch 블록은 if문처럼 순차적으로 수행됨
			
			String str = null;
			System.out.println(str.length()); // NullPointerException 예외 발생 위치
			// 예외가 복수개일 때 각각의 예외를 별도로 처리해도 되지만
