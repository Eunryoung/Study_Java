...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * 	  => 외부에서 인스턴스 생성 없이도 접근할 수 있도록(외부에서 인스턴스 생성이 불가능해야하므로) static 키워드 사용
		 * 	  => 외부에서 함부로 접근하지 못하도록 private 접근 제한자 선언
...
//		SingletonClass sc2 = new SingletonClass();
		...
		// 해당 변수를 통해 인스턴스의 주소를 가져올 수 있다!(접근할 수 있다!)
...
//		sc.instance = null;
		...
		// 변수 instance에 저장했으나, private 접근제한자로 인해 접근 불가능하므로
...
//	public SingletonClass() {}
	...
	// => 외부에서 인스턴스 생성없이 접근해야하므로(외부에서 인스턴스 생성이 불가능해야하므로) static 키워드 사용
	// => 외부에서 함부로 접근하지 못하도록 private 접근 제한자 선언
	...
	// => 인스턴스 생성없이도 접근해야하므로 static 키워드 사용
	public static SingletonClass getInstance() { // 리턴타입이 기본 데이터타입이 아닌 SingletonClass 클래스 타입임
		return instance;
	}
