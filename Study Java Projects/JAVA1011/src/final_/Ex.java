...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 	     = 상수로 취급됨(= 리터럴, Constant)
		 * 	  => 일반 변수와 구별하기 위해 이름을 보통 대문자로 사용하며,
		 * 		 단어와 단어 사이의 구분은 언더스코어(_)를 사용함(e.g. MAX_NUM)
		 * 	  => 반드시 초기화 필수!
		 * 		 단, 초기화를 수행하지 않는 상수를 blank final 상수라고 하며
		 * 		 생성자를 통해 초기화해야 사용이 가능함
...
//		Math.PI = 3.14; // 컴파일 에러 발생! 값 변경 불가!
		...
		// => final 클래스인 String 클래스를 상속할수는 없지만 new 키워드로 인스턴스는 만들 수 있다
...
//	final int finalMember;
	...
	final int finalMember = 10; // 초기화해서 오류 해결
	
	// 만약, final 상수를 초기화하지 않을 경우
	final int blankFinalMember; // 위와 같은 이유로 컴파일 에러 발생!
	// 단, 생성자를 통해 초기화하는 경우 사용이 가능하다!
	
	public FinalMember(int blankFinalMember) { // 파라미터 생성자
		super(); // Object 클래스의 기본 생성자 호출
		this.blankFinalMember = blankFinalMember; // blank final 상수 초기화
		// 위에서 초기화 하지않은 final 상수를 생성자 내에서 초기화하면 오류가 사라짐
		// blank final 상수의 경우 인스턴스 생성시마다 파라미터 값을 다르게 줄 수 있음! 
	}
...
class FinalMethod {
	public final void finalMethod() { // final 키워드 붙은 메서드 = 오버라이딩 불가
		System.out.println("슈퍼클래스의 finalMethod()");
	}
	
	public void normalMethod() { // 일반 메서드는 오버라이딩 가능
...
//	}
	// => 직접 재정의(오버라이딩)는 할 수 없지만 다른 메서드에서 final 메서드 호출은 가능하다!
