...
//		p.parentPrn(); // 호출 가능(자신의 멤버)
//		p.childPrn(); 
		// => 오류 발생! The method childPrn() is undefined for the type Parent
		// => 서브클래스에서 직접 정의한 메서드는 호출 불가!
		...
		c.parentPrn(); // 호출 가능 -> 슈퍼클래스에서 상속받은 메서드 
		c.childPrn(); // 호출 가능 -> 서브클래스에서 직접 정의한 메서드 
		...
		Parent p; 
		// => Parent 클래스의 인스턴스 생성해 오류 발생! -> 위로 올라가서 주석 처리할 것!
		
		// 슈퍼클래스 타입 변수 p에 서브클래스 타입 Child 인스턴스 주소를 전달(대입)
		p = c; // Parent p = new Child();
		// => 묵시적 형변환(업캐스팅) : Parent p = (Parent)c;
		// 자식 데이터 타입을 부모 데이터 타입으로 변환할 경우 자동형변환 가능
...
//		p.childPrn(); // 서브클래스에서 직접 정의한 메서드(접근 불가) => 오류 발생!
		 ...
		 * 	 실제 실행 시점에서 오류(= 실행 오류 = 런타임 오류)가 발생하게 됨
...
//		c2 = (Child)p2; 
		// =>  Parent -> Child 타입으로 강제 형변환 = 다운캐스팅(오류 발생!)
...
//		c2.childPrn(); 
		// => 실제 Parent 인스턴스에 존재하지 않는 메서드
		//    (여기서 오류 나기전에 다운캐스팅에서 먼저 오류 발생!)
