...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		int num1 = 10, num2 = 20; // 기본 데이터타입 변수 선언 및 초기화
		Integer n1 = 10, n2 = 20; // Wrapper 클래스 타입 변수 선언 및 초기화
		
		// max() : 두 정수의 최대값을 계산하는 메서드
		System.out.println(Integer.max(num1, num2)); // 20 
		System.out.println(Integer.max(n1, n2)); // 20 
		// => 기본 데이터타입과 Wrapper 클래스 타입 모두 사용 가능
		
		// min(): 두 정수의 최소값을 계산하는 메서드
		System.out.println(Integer.min(num1, num2)); // 10
		System.out.println(Integer.min(n1, n2)); // 10
		// => 기본 데이터타입과 Wrapper 클래스 타입 모두 사용 가능
		...
		// => 주로 특정 문자에 대한 판별(Character.isXXX())을 수행하거나
...
//			System.out.println("소문자로 변환한 결과 : " + ch);
			...
			// Character.isSpace(ch) 사용 X => 취소선 처리되어 있음
