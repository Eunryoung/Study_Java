...
//		map.put(3, "Spring framework");
		System.out.println("3을 키로 갖는 값 추가 : " + map.put(3, "Spring framework")); // 리턴 되는 값 : Android(= 제거되는 데이터) 
		System.out.println("map 객체의 모든 엔트리 : " + map);
		
		// 키는 중복이 불가능하지만, 값은 중복이 가능함
		System.out.println("4를 키로 갖는 Spring framework 값 추가 : " + map.put(4, "Spring framework")); 
		// => 키가 중복되지 않으므로 제거되는 데이터가 없기 때문에 null이 리턴됨
		System.out.println("map 객체의 모든 엔트리 : " + map);
		
		// Object get(Object key) : key에 해당하는 값 리턴(없을 경우 null)
		System.out.println("정수 2(key) 에 해당하는 키의 데이터(value) : " + map.get(2)); // JSP
		System.out.println("문자열 2(key) 에 해당하는 키의 데이터(value) : " + map.get("2")); // null
		
		// Set keySet() : Map 객체 내의 모든 키 리턴 -> Set 타입 객체로 리턴
		Set keySet = map.keySet();
		System.out.println("모든 키 : " + keySet); // [1, 2, 3, 4]
		
		// Collection values() : Map 객체 내의 모든 값 리턴
//		List valueList = map.values(); // 오류 발생
		// => Type mismatch: cannot convert from Collection to List
		// => 강제 형변환 실행...?
//		List valueList = (List)map.values(); // 주의! 직접 다운캐스팅 불가!
		List valueList = new ArrayList(map.values()); // 객체 생성 파라미터로 전달(업캐스팅)
		...
		System.out.println(entrySet); // [1=자바, 2=JSP, 3=Spring framework, 4=Spring framework]
		...
		map2.put("010-222-3333", "홍길동"); // 이름(값)은 중복 가능
